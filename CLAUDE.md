...
>
> **Status (2026-07-17):** Round 1 + Round 2 + Round 3 shipped and merged to `main` (Round 3 = person-a UI + person-b schema/APIs + person-c integrations; full suite green incl. 28 RLS tests on Postgres, build clean, no client-bundle secret leak). **Round 4 is planned:** live backend - take the fixture-first app to a real hosted Supabase project (SSR auth sessions, RLS verified on the real DB, live Realtime + Storage, Vercel deploy). No new product features. Two-way split A/B on branches `round4-person-a` / `round4-person-b`; seams in `docs/FOUNDATION-CONTRACT.md` section 14; per-person plans in `docs/IMPLEMENTATION-PERSON-{A,B}-ROUND4.md`. Build/feature status is tracked in `docs/PROGRESS.md`.
