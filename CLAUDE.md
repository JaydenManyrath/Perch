...
>
> **Status (2026-07-17):** Round 1 + Round 2 shipped and merged to `main`. Round 3 is integrated and verified on the current release-candidate branch: automated gates and local Supabase reset/RLS/seed pass, and fixture-browser acceptance passes except for real Mapbox marker clicks because no public token was available. It is not yet merged to `main`; see `docs/SPRINT-3-ACCEPTANCE.md` and `docs/PROGRESS.md` for the exact evidence and remaining gate. The shared interface lives in `docs/FOUNDATION-CONTRACT.md` section 13; the per-person Round 3 docs are historical implementation plans, not current status.
