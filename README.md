...
- Round 3: UI (person-a) and schema + core APIs (person-b) are shipped; integrations + AI (person-c) remain in progress. Plans live in `docs/IMPLEMENTATION-PERSON-{A,B,C}-ROUND3.md`.
  - UI (person-a, DONE): event images with a placeholder fallback on the feed, a comprehensive perch detail sheet (furnished, pros, bed/bath/sqft, amenities, utilities) with matching post-form inputs, a full booking flow (request -> owner approves -> confirmed -> listing leaves the deck), roommate grouping (invite friends onto a booking), a deterministic finance breakdown (take-home vs salary, COL-adjusted rent ceiling, upfront cash needed with stipend/bonus offset) rendered in onboarding + on landing + as a per-perch affordability line, a fuller checklist grouped by category (travel, logistics, packing, admin), onboarding percentages removed (confidence -> "check this" flag, step dots replace the percent bar), and richer info sheets on map markers (listing, sticker, and event with attendance + taste).
  - Schema + core APIs (person-b, DONE): migrations 0011/0012 - listing detail columns (furnished, pros, bed/bath/sqft, amenities, utilities), a `bookings` table with a deterministic state machine (requested -> approved -> booked; a booked listing goes taken and leaves the deck; decline/cancel release), roommate grouping (roommate_ids + invites, friend-or-invited enforced in code + RLS), a `cost_of_living` table, and finance inputs on `users`. New routes: `GET /api/listings/{id}` (comprehensive ListingDetail), `POST /api/listings/{id}/book`, `POST /api/bookings/{id}/approve|decline|confirm`, `POST /api/bookings/{id}/roommates` + `/accept`, `GET /api/bookings`, `GET /api/finance` (deterministic take-home via progressive brackets + FICA + state estimate, COL-adjusted budget, upfront cash, stipend/bonus). The negotiation budget scout now uses take-home + cost-of-living, never raw salary. Feed and nearby-events guard `datetime >= now` (upcoming only). Adversarial RLS cases cover bookings/roommates/cost_of_living, alongside finance + booking-state-machine unit tests.
