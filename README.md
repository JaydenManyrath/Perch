...
- Round 3: RELEASE CANDIDATE VERIFIED on 2026-07-17. The integrated UI, schema/APIs, integrations, and parser work are present together. The focused Sprint 3 suite passed all 155 tests, and the full suite passed 325 of 326 tests across 46 passing files with 1 environment-gated live-auth test/file skipped. Local reset/RLS/seed passed, and the production build passed. Fixture-browser acceptance passed event images, comprehensive listing detail, booking request/approval/confirmation with immediate deck removal, roommate invite acceptance/grouping, canonical finance, grouped checklist, and percentage-free onboarding. A real Mapbox marker-click pass remains unverified because no `NEXT_PUBLIC_MAPBOX_TOKEN` was available. This release candidate is not yet merged to `main`; do not represent it as published. See [SPRINT-3-ACCEPTANCE.md](docs/SPRINT-3-ACCEPTANCE.md).
