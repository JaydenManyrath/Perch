...
- RC34 (optional) Map place-details lookup. Closed 2026-07-17 as not needed for Sprint 3. Person A's RA38 richer map sheets meet their accepted behavior with existing fixtures, Person B marker payloads, and existing detail/feed routes. No documented Person A or Person B request requires external Mapbox place details, so do not add a route, key, provider, or abstraction.
...
- Secrets: TICKETMASTER_API_KEY (exists), any future cost-of-living key, in gitignored .env; documented in docs/SECRETS.md and .env.example. RC34 is closed as not needed, so no Mapbox place-details key is introduced. Rate-limit any new route.
...
- Phase R3C-4 place-details (RC34): closed 2026-07-17 as not needed; A did not request external details, and RA38 uses existing payloads.
