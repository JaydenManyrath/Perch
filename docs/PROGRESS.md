...
## Round 4 - Live backend: Supabase provisioning + go-live (planned 2026-07-17; audited on main 2026-07-20)

Two-way split (no new product features): take the fixture-first app to a real, logged-in,
deployed Supabase backend. Branches: round4-person-a, round4-person-b (each
restarted from main).

### Client / auth session / deploy (person-a)
- [done 2026-07-20] RA41 middleware.ts: SSR auth session refresh (@supabase/ssr) + matcher; keeps a live session fresh across navigation (fixture-safe no-op when unconfigured). Evidence: middleware.ts on main (createServerClient + auth.getUser() refresh, static-asset-excluding matcher, hasSupabase() no-op) + lib/auth/middleware.test.ts.
- [done 2026-07-20] RA42 Auth flow: login persists a real session, sign-out, protected-route redirects, a current-user hook from the live session. Evidence: app/login/page.tsx (signInWithPassword, perch-demo password seam), components/auth/SignOutButton.tsx, middleware redirects (protected -> /login, signed-in /login -> /feed), lib/auth/session.tsx + lib/auth/server-session.ts wired in app/(shell)/layout.tsx. Live smoke tests/auth-live.test.ts PASSED against the hosted project 2026-07-20 (intern0@perch.demo signs in and reads its RLS-scoped profile).
- [done 2026-07-20] RA43 Fixture-to-live flip: every lib/data/source.ts getter hits live on DATA_SOURCE=live and falls back to fixture on error (no broken screens). Evidence: canUseLiveData() gate + live-then-fixture in every getter (spot-checked getFeed/getMatches/getMapPlaces/getItinerary/parseOffer/getMe/getListings/insertMessage); DATA_SOURCE_GETTER_AUDIT inventory enforced by lib/data/source-live.test.ts.
- [done 2026-07-20] RA44 Realtime DMs live against the hosted project (reuse the optimistic reconcile). Evidence: lib/hooks/useRealtimeMessages.ts subscribes postgres_changes INSERT on messages filtered by conversation_id, reconciles echoes over optimistic sends; live insertMessage derives sender_id from the session. Code-verified; a two-browser live DM session has not been exercised in this audit.
- [done 2026-07-20] RA45 Mapbox live token wiring (map renders real tiles; placeholder fallback kept). Evidence: lib/env.ts hasMapbox() + components/map/MapCanvas.tsx sets mapbox-gl accessToken from NEXT_PUBLIC_MAPBOX_TOKEN, with a no-token/load-failed placeholder and an 8s load timeout. Token present in local .env.local; the RA38 real-browser marker-click pass is still not on record.
- [done 2026-07-20] RA46 Storage upload UI: listing photos + profile avatar to the Supabase bucket via public URLs. Evidence: lib/storage/image-upload.ts (owner-prefixed path into the migration-0005 listing-photos bucket, failure-as-value), components/storage/ImageUploadField.tsx, components/post/PostListingForm.tsx (photos), components/profile/ProfileAvatar.tsx + app/onboarding/_steps/AvatarStep.tsx (upload + saveProfileAvatar persists users.avatar_url when Supabase is configured; local preview + one-tap skip kept).
- [blocked 2026-07-20] RA47 Vercel deploy: still no preview URL or hosted smoke evidence. No .vercel project link exists in this checkout. The Supabase half is no longer blocking (hosted project is migrated + seeded; live auth smoke passed 2026-07-20); what remains is owner-dashboard work: create/link the Vercel project, set env vars (see docs/PRODUCTION-READINESS.md), deploy, and run the deployed-URL smokes. Fixture mode remains the working fallback.
...
Hosted status (2026-07-20): the hosted project exists, is migrated, and is seeded -
`.env.local` carries the project URL + anon + service-role keys, the
`tests/auth-live.test.ts` login smoke passes against it, and 39 live Ticketmaster
events are in its events table (54 total). Still pending: the hosted adversarial RLS
run (`RUN_RLS_TESTS=1 RLS_TEST_DATABASE_URL="$SUPABASE_DB_URL" npm run rls:test`)
needs `SUPABASE_DB_URL` (the direct Postgres connection string), which is not in
`.env.local` on this machine. See docs/PRODUCTION-READINESS.md.
...
flight. Branches: round5-person-a, round5-person-b, round5-person-c,
round5-person-d (each cut from main).
...
- [done] RB51 Shared ingest core (lib/events/ingest.ts) + guarded /api/cron/ingest-events route (CRON_SECRET, 401 otherwise); script is now a thin wrapper
- [done] RB52 vercel.json crons (ingest-events + expire-listings, both daily/Hobby-floor) + cooldown-gated (6h/city) fire-and-forget on-request refresh with in-flight guard; rate math documented in code
- [done] RB53 Retired the workflow's Ticketmaster step (seed stays). Deployed-URL check pending deploy of this branch + CRON_SECRET in Vercel env (deploy-owner seam); verified locally instead (see log)
...
- 2026-07-20: Round 7 merged to main (r7-events-quality + r7-live-freshness + r7-account-first). Events: image precedence now falls back beyond 16_9 (largest non-fallback any-ratio, then fallback images; null only with no url) so real events stop rendering placeholders, and all event surfaces open the real Ticketmaster page in a new tab. Freshness: every ingest pass (Vercel cron, GH Action, cooldown refresh) prunes ticketmaster rows older than 6h past start, all event-serving routes guard upcoming-only in-query (fixed the friends-notes strip resurfacing passed events), nothing hardcodes a date - always upcoming relative to now. Accounts: getMe/getUserById normalize live rows (minted accounts with NULL optional fields no longer bounce to the seeded persona on any of the 7 current-user surfaces) and onboarding shows a visible account-creation step (creating -> created with the minted email, or a plain fallback note - no silent swap). Gates on the merged tree: 492 tests, typecheck, lint, production build, unpiped bundle secret greps all clean.
...
- 2026-07-16: Round 2 fully shipped and merged to main. Implementation docs removed; keeping ARCHITECTURE.md, PROGRESS.md, SECRETS.md, README.md, docs/ARCHITECTURE.md.
...
- 2026-07-20: Round 5 Person D (RD51-RD53) built on branch round5-person-d. Bird-word nav labels with plain subtitles (Flyway/Perches/Migration/Chirps/Nest), routes frozen; DMs + Map headers swept; a flat-vector BranchMotif on emotional surfaces only (SideRail edge, onboarding backdrop, Chirps empty state, login/landing) and deliberately absent from decision surfaces; glossary + README updated. Verified in isolation on the round-5 planning base: typecheck + lint clean, full test suite green, production build clean. Not merged to main.
- 2026-07-20: Round 5 merged to main (RA51-RA53, RB51-RB53, RC51-RC53, RD51-RD53) in four merge commits; full suite, typecheck, and lint green on the integrated tree. All round4/round5 branches deleted local + remote. Follow-ups on main: the branch motif moved off the SideRail to a faint backdrop behind page content; the feed is now area-scoped - /api/feed resolves the viewer's onboarding city (known-city table -> Mapbox geocode -> Seattle default), serves only events within the city radius (falling back to all upcoming rather than an empty feed), and kicks the same cooldown-gated Ticketmaster refresh as /api/events/nearby so the events page fills with live Ticketmaster events for the viewer's area once the deployed key is exercised.
- 2026-07-20: Onboarding mints the account for the person ON the offer letter, with a fresh social graph. The parser (heuristic + LLM schema + verification) now extracts the candidate's name as a first-class OfferParse field (salutation/labelled patterns; null + flagged for review when absent; LLM value must be grounded in the source text); the correction UI exposes it. POST /api/onboarding/account (anonymous by design, IP rate-limited) creates the auth user (<name-slug>@perch.demo with the shared perch-demo password seam, suffix on collision) and a users row from the parse (identity + offer money fields, verified=false), writing NO friendships/conversations - a new account has friends only when they add them. The browser signs into the new account after the offer step; fixture mode mirrors it (the in-memory "me" takes the letter's identity, avatar resets to initials when the name differs, and the seeded friend graph empties). Done step shows the minted email for future logins.
...
- 2026-07-20: Production-readiness audit (branch r6-prod-audit). Round 4 RA41-RA46 confirmed shipped on main and flipped to done with per-item evidence; RA47 stays blocked on the Vercel project link + dashboard env. Fixed the one gate breaker found: next lint failed on main because lib/events/ticketmaster.ts carries an eslint-disable for @typescript-eslint/no-var-requires while the @typescript-eslint plugin was never registered - registered the plugin in .eslintrc.json (no rules enabled, zero behavior change; the owned file was not touched). Wired nothing else: AvatarStep already persists avatars through uploadStorageImage + saveProfileAvatar. Gates on this branch: vitest 471 passed / 7 skipped; tsc clean; next lint clean; next build clean; both client-bundle secret greps (sb_secret, SUPABASE_SERVICE_ROLE_KEY in .next/static) empty on a build run with the real keys in env. Live smokes run against the hosted project: auth-live login smoke passed; events count read back 39 ticketmaster / 54 total. Remaining go-live work consolidated in docs/PRODUCTION-READINESS.md.
- 2026-07-20: Ticketmaster ingestion fixed on main (commit 54df783): Discovery timestamps with real milliseconds no longer 400 (stripped before the API call), upserts use deterministic primary-key ids because the partial unique index cannot be an ON CONFLICT target, and ticketm.net images are allowed. 39 real Ticketmaster events were ingested to the hosted project on 2026-07-20 (count re-verified read-only during the audit).
- 2026-07-20: Round 5 person-b (RB51-RB53) built on branch round5-person-b. The deployed app now owns event freshness: lib/events/ingest.ts is a shared, idempotent (upsert on unique (source, external_id)) ingest core reused by the CLI script, the new guarded GET /api/cron/ingest-events route (Authorization: Bearer CRON_SECRET, 401 otherwise), and a cooldown-gated (6h/city, in-flight guard) fire-and-forget on-request refresh on /api/events/nearby. vercel.json schedules ingest-events + expire-listings daily (Hobby-plan floor; expire-listings gained a GET handler + shared cron auth so Vercel Cron can drive it, POST kept for on-demand). The seed-demo workflow's Ticketmaster step is removed (seed kept). Rate math (documented in code): cron 1/day/city + <=4 on-request refreshes/day/city = <=5, far inside Ticketmaster's 5000/day. Server + config only; no schema, contract, or UI changes. Verification (at this branch state): typecheck 0, next lint clean, next build 0 (only the pre-existing lib/parsers/ocr.ts OCR warning), full suite 378 passed / 6 skipped. Runtime smoke on next start against the real hosted Supabase: cron/ingest-events 401 without/with-wrong bearer, 200 with correct bearer and with the legacy x-cron-secret (no TICKETMASTER_API_KEY -> source=fallback, 0 upserted, no crash); expire-listings GET 403 without bearer, 200 with (ran the real freshness pass), POST legacy header 200. Not yet verifiable here: real upcoming events with images (needs TICKETMASTER_API_KEY, set in Vercel) and the deployed-URL check (needs this branch deployed + CRON_SECRET in Vercel Production+Preview).
